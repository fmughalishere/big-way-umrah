    ...
    <main className="bg-white min-h-screen">
      ...
      <div className="pt-48 pb-24 flex flex-col items-center px-6 md:px-10 max-w-[1440px] mx-auto">
        ...
        <motion.div
          ...
          className="mt-24 w-full max-w-5xl rounded-[60px] overflow-hidden shadow-[0_30px_100px_rgba(59,89,152,0.15)] border border-slate-100"
        >
          <div className="px-10 md:px-20 py-10" style={{ backgroundColor: blueColor }}>
            <p className="text-white font-black text-2xl uppercase tracking-widest">Agency Application Form</p>
            <p className="text-white/60 text-sm font-medium mt-1">All fields are required. Reviewed within 24–48 hours.</p>
          </div>
          <div className="bg-white px-10 md:px-20 py-16 relative overflow-hidden">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8">
              ...
              {formFields.map((field) => (
                <div key={field.name} className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border-2 border-slate-100 p-5 rounded-2xl text-slate-800 outline-none placeholder:text-slate-400 font-bold text-sm
              ...
              <div className="space-y-3 md:col-span-2 mt-2">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                  {["Umrah Packages", "Ticket Booking", "Hajj Packages", "Visa", "Transportation"].map((service) => (
                    <label
                      key={service}
                      className="flex items-center gap-3 bg-slate-50 border-2 border-slate-100 p-4 rounded-xl cursor-pointer hover:border-[#3b5998] hover:bg-blue-50/40 transition-all duration-300"
                    >
                      <input
                        type="checkbox"
                        value={service}
                        className="w-4 h-4 rounded accent-[#c1272d]"
                      />
              ...
              <button
                type="submit"
                className="md:col-span-2 mt-10 text-white py-7 rounded-[30px] font-black tracking-[4px] uppercase text-xs
              ...
              <p className="md:col-span-2 text-center text-slate-400 text-[9px] font-bold tracking-[2px] uppercase mt-4">
